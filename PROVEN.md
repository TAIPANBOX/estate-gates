...
| SUP-4: Scorecard published for all 21 repositories, read from api.scorecard.dev after the tag-object pins were fixed and every repository's push run uploaded: **Security-Policy 10 everywhere**; Signed-Releases tokenfuse 3 (v0.5.0 signed with provenance, the two earlier releases in the window not), trailryx, idryx, qryx, agent-stack-go, mockryx 0 (no release since wave 1), terraform-provider 8; Pinned-Dependencies 10 in five repositories and 7 to 8 in most others (container base images not pinned by digest); **Token-Permissions 0 in ten repositories** (a top-level `contents`/`packages: write` in release.yml, or a workflow with no top-level `permissions:`), 10 in eleven | 2026-09-13 | api.scorecard.dev, each repository's `scorecard.yml` push run after the pin merge | `go-to-market-2026-09/evidence/1.0/r0-mac-2026-09-13/sup-4-scorecard-sunday.log` (the table, the check details for tokenfuse, idryx and trailryx, and the reading) | the score climbs one signed release at a time (Scorecard reads the last five), so "Signed-Releases 10" is not a figure a 1.0 tag reaches on its own; the Token-Permissions fix is a mechanical follow-up in ten repositories, not done today. Until this afternoon every upload had been refused as an "imposter commit" (the C21 story) |
