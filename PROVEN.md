...
| R3: RUN-2 (the wrong door for the wire), REC-5 (the D6 set), a real OpenAI key, Nova on Bedrock's OpenAI door, the fix for tokenfuse#283 | RUN-2 and REC-5 were not run on R3; no OpenAI key exists (OpenRouter stood in); Bedrock does not serve Nova as OpenAI wire on this account; #283 is filed, not built | 2026-09-13 |
